...
// DB types regenerated by `npm run supabase:types` after the
// 20260517000000_linkedin_llm_metadata.sql migration. Until then, the new
// columns (file_hash, last_export_path, parsed_json, parser_model,
// scorer_model, prompt_version) aren't on the generated types — cast loosely.
// Remove the casts after regen.
...
export const parseLinkedinFn = inngest.createFunction(
  {
    ...
    onFailure: async ({ event, error }) => {
      ...
      await (supabase.from("linkedin_profiles") as LinkedinTable)
        .update({
          status: "failed",
