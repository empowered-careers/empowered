...
- [ ] **P2-5 Expanded assessments** — the granular four (~~Role Clarity~~, ~~Values~~, ~~Strengths~~, ~~Leadership~~) + Mindset/Saboteur + Communication; re-add `mindset_score` / `communication_score` to `candidate_scores`. **Two have come out of this bundle early** — Big Wins built 2026-08-14 as a resume-rewrite surface, not a score-producing assessment (`docs/big-wins-implementation-plan.md`); **Role Clarity built 2026-08-19** and it _does_ produce a score, owning `candidate_scores.role_clarity_score` (`docs/role-clarity-spec.md`). **Values, Strengths and Leadership are cancelled, not pending** (2026-08-19) — the Career Identity Blueprint already scores all three: `leadership_style` + `result.leadership`; `culture_values` / `company_stage_fit` / `structure_vs_autonomy` + `redLight`; `greenLight` / `voiceProfile.green_light_strengths`. A separate quiz would re-ask the same 15 questions and produce a second, conflicting `leadership_style`. Their coming-soon cards were deleted from `/assessments`; any future surface is a drill-down on existing Blueprint axes, not a new questionnaire.
